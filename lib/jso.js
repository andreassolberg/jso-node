...
var OAuth = function(config, storageStrategy, redirectStrategy) {
...
OAuth.prototype.getMiddleware = function () {
	return new OAuthConnectMiddleware(this);
}

OAuth.prototype.getRedirectHandler = function(req, res) {
...
OAuth.prototype.resolveCode = function(code, callback) {
...
OAuth.prototype.callbackCode = function(code, state, callback) {
	...
	this.resolveCode(code, function(codeResponse) {
		 ...
		 */
		if (!atoken.state && that.hasOption('default_scope')) {
			state.scopes = that._getRequestScopes();
		...
		if (atoken["expires_in"]) {
			...
			atoken["expires"] = now + that.getOption("default_lifetime");
		} else if (instance.config["permanent_scope"]) {
			if (!this.store.hasScope(atoken, that.getOption("permanent_scope", '___'))) {
...
OAuth.prototype.checkToken = function(callback, redirectCallback, opts) {
...
OAuth.prototype.getToken = function(callback, redirectCallback, opts) {
	// var scopesRequest  = this._getRequestScopes(opts);
	var scopesRequire = this._getRequiredScopes(opts);
	...
	if (token) {
		...
		this._authorize(callback, redirectCallback, opts);
	}

}

OAuth.prototype._authorize = function(callback, redirectCallback, opts) {
	var 
		request,
		authurl,
		scopes;
...
OAuth.prototype.hasOption = function(key, overrides) {
...
OAuth.prototype.getOption = function(key, defaultValue, overrides) {
...
OAuth.prototype.getJSON = function(url, options, callback, redirectCallback) {
	...
	this.getToken(function(token) {
		...
		if (that.getOption('presenttoken', null, options) === 'qs') {
			requestOptions.url += ((url.indexOf("?") === -1) ? '?' : '&') + "access_token=" + encodeURIComponent(token["access_token"]);
		} else {
